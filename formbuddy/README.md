...
FormBuddy defaults to `http://localhost:8081` (same as SparX’s usual `llama-server` port). If you use another host, port, or **https**, set:

`export FORMBUDDY_LLM_BASE_URL=http://127.0.0.1:YOUR_PORT`

If the URL/port is wrong, `httpx` may raise `RemoteProtocolError: illegal request line` because the peer is not speaking HTTP/1.1 there (or you used `http://` against a TLS-only port).

You can still edit `config.py` for `LLM_MODEL` / `LLM_OPTIONS`, or set `FORMBUDDY_LLM_MODEL` for the model id.
